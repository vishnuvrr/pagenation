...
function displayData(currentPageNumber)
{
  var footerSize=data.length/20;
  console.log(currentPageNumber);
  document.getElementById("content").innerHTML="";
  for (let index = 0; index < footerSize; index++) {
    var page=document.createElement("div");
    var text=data[(currentPageNumber-1)*5+index];
    page.innerHTML=`<h2>${text.name}</h2> <h5>${text.email}</h5>`;
...
function createFooter()
{
  for (let index = 1; index <= 20; index++) {
    ...
    page.addEventListener("click",displayData.bind(index,index));
